...
import cn.bugstack.domain.agent.model.valobj.enums.AiClientTypeEnumVO;
...
public class Step1AnalyzerNode extends AbstractExecuteSupport{
    ...
    // 执行分析功能的client。任务分析 -> 获取client并执行 -> 解析执行结果 -> 将结果存入上下文中 -> 检查是否完成 -> 下一节点
    @Override
    ...
    private void sendAnalysisSubResult(DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext,
        ...
        if (!subType.isEmpty() && !content.isEmpty()){
            AutoAgentExecuteResultEntity result = AutoAgentExecuteResultEntity.createAnalysisSubResult(dynamicContext.getStep(), subType, content, sessionId);
            sendSseResult(result,dynamicContext);
        }
    }
