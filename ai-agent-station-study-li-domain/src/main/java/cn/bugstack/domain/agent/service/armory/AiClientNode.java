...
import cn.bugstack.domain.agent.model.valobj.AiClientSystemPromptVO;
import cn.bugstack.domain.agent.model.valobj.AiClientVO;
...
public class AiClientNode extends AbstractArmorySupport{
    ...
    protected String doApply(ArmoryCommandEntity requestParameter, DefaultArmoryStrategyFactory.DynamicContext dynamicContext) throws Exception {
        log.info("Ai Agent 构建，ai client 构建节点 {}", JSON.toJSONString(requestParameter));

        List<AiClientVO> aiClientVOList = dynamicContext.getValue(dataName());
        ...
        for (AiClientVO aiClientVO : aiClientVOList) {
            ...
            registorBean(beanName(aiClientVO.getClientId()), ChatClient.class, chatClient);
        }
